defmodule Gossip.FactMonger do
  ...
  def run(neighbours, our_sum, our_weight, our_round_counter, our_pid, ticker_pid) do
    #IO.puts "Starting fact monger"
    receive do
      {:tick, _index} = message ->
        #IO.puts (inspect(message))
        #IO.puts (inspect(neighbours))
        #IO.puts ("Sending fact to one of neighbours of #{inspect(our_pid)}")
        if !is_nil(neighbours) and !Enum.empty?(neighbours) do

          # Send periodically to one of the neighbours
          send(Enum.random(neighbours), {:pushsum, our_sum/2, our_weight/2, our_round_counter, our_pid})
      ...
      {:last_tick, _index} = message ->
        IO.puts (inspect(message))
        IO.puts ("-----------Removing timer for fact monger and terminating fact mongering for #{inspect(our_pid)}-------")
        :ok

      {:new_sum_and_weight, new_sum, new_weight, new_round_counter, our_new_pid} ->
        #IO.puts "Updating sum to #{new_sum} and weight to #{new_weight} for FactMonger and periodically sending it to new neighbours of #{
            #inspect(our_new_pid)
          #}"
