defmodule Gossip.P2PSupervisor do 
  ...
  def start_children(supervisor, num_nodes, topology \\ "full", algorithm \\ "gossip") do
    ...
    # send_fact(child_pids, {:gossip, "The answer to the question is 42"})
    {:ok, child_pids}
  end

  def send_fact(child_pids, {:gossip, fact, default_fact_counter, pid}) do
    random_child_pid = Enum.random(child_pids)
    send(random_child_pid, {:gossip, fact, default_fact_counter, pid})
  end
  ...
  defp initiate_algorithm(algorithm, child_pids) do
    case algorithm do
      "gossip" -> send_fact(child_pids, {:gossip, "The answer to the question is 42", 0, self()})
